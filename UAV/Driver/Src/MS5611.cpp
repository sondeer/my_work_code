...
void Ms5611Ctrl::Reset(void)
{
  ...
  HAL_Delay(20);
	...
	float* p = &this->calibRef.fData;
	uint16_t buff[8];
  for (i=0; i<PROM_SIZE; i++)
  { 
    this->ReadRegister(CMD_PROM_READ + (i*2),buff + i, 2);
		p[i] = buff[i] / 65536.0f;
  }
	...
	HAL_Delay(10);
	temp = this->GetPressure();
	/*
	for(i = 0 ; i < 500 ; i ++)
	{
		this->GetPressureKF(temp,0.02f);
	}*/
}
...
void Ms5611Ctrl::ConvertionPressure(void)
{
	...
	uint32_t D2;
float  Ms5611Ctrl::GetTemperature(void)
{
	//uint32_t D2;
	this->ReadRegister(CMD_ADC_READ, &D2, 3);
    
  this->dT = D2 / 65536.0f - calibRef.tRef * 256;
  this->temperature = 2000 + this->dT * calibRef.tSens * 512; 
	return  this->temperature / 100;
}

	 uint32_t D1;
	 float  off, sens,tmp,off2,sens2,t2; 

float  Ms5611Ctrl::GetPressure(void)
{
...
//	 int64_t  off, sens;
   ...
   off  = this->calibRef.pOff * 0x10000 + this->calibRef.tCO * this->dT * 512;
   sens = this->calibRef.pSens * 0x8000 + this->calibRef.tCS * this->dT * 256;
	
   if (this->temperature < 2000)
   {
      t2    = dT * dT / 0x8000;
      tmp   = (this->temperature - 2000);
      tmp   = tmp * tmp / 65536.0f;
      off2  = (5 * tmp) / 2;
      sens2 = (5 * tmp) / 4;
   ...
   this->pressure = ((D1 / 65536.0f) * sens) / 16  - off * 2;     
...
void Ms5611Ctrl::SetPressureBase(void)
{
	this->pressureBase = this->pressureKF;
}

float Ms5611Ctrl::GetHeight(void)
{
	return (this->pressureBase - this->pressureKF) / 12.7f;
}
