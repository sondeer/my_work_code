...
UartDevice::UartDevice(int port)
{
	tx_headIndex = tx_tailIndex = rx_tailIndex = 0;
}

void UartDevice::Init(int baud)
{	
  ...
  HAL_NVIC_EnableIRQ(DMA1_Channel2_3_IRQn);
	...
	HAL_DMA_Start(&hdma_rx,(uint32_t)&USART1->RDR,(uint32_t)rx_dataBuff,BUFF_SIZE);
...
int UartDevice::DataLength(void)
{
	int rx_headIndex = BUFF_SIZE - DMA1_Channel3->CNDTR;
...
int UartDevice::ReadData(void* buff,int size)
{
	int rx_headIndex = BUFF_SIZE - DMA1_Channel3->CNDTR;
	int leftData = rx_headIndex - rx_tailIndex;
	if(leftData < 0)
...
int UartDevice::WriteData(void* buff,int size)
{
	/*
	if(DMA1_Channel2->CCR & DMA_CCR_EN)
	{
		int rvtx_tailIndex = tx_tailIndex - DMA1_Channel2->CNDTR;
	...
	else 
	{
		...
		BeginTransmitDMA(tx_dataBuff,size);
	...
	*/
	...
	HAL_UART_Transmit(&UartHandle, (uint8_t *)buff, size, 100);
	return 0;
}

void UartDevice::InttruptCallBack(void)
{
	DMA1->IFCR = DMA_FLAG_GL2 | DMA_FLAG_TC2 | DMA_FLAG_HT2 | DMA_FLAG_TE2 ;
	...
	{
		BeginTransmitDMA(tx_dataBuff + tx_tailIndex,tx_headIndex - tx_tailIndex);
		SET_BIT(USART1->CR3, USART_CR3_DMAT);
		tx_tailIndex = tx_headIndex;
	}
	else 
	{
		BeginTransmitDMA(tx_dataBuff + tx_tailIndex,BUFF_SIZE - tx_tailIndex);
		SET_BIT(USART1->CR3, USART_CR3_DMAT);
		tx_tailIndex = 0;
	}
}

void UartDevice::BeginTransmitDMA(uint8_t *buff,int size)
{
	DMA1_Channel2->CCR &= ~DMA_CCR_EN;
